...
public class CraftEvent implements Listener {
    ...
    public void run(PlayerInteractEvent event) {
        ...
        if (event.getPlayer().getWorld().getName().equalsIgnoreCase(Worlds.SPLEEF) && event.getPlayer().getGameMode() == GameMode.SURVIVAL && event.getItem() != null) {
            if (event.getItem().getType() == Material.STICK && event.getPlayer().getInventory().contains(Material.SNOW_BALL, BLOCK_COST)) {
                ...
                if (DataPlayer.getUser(event.getPlayer()).remove(Material.SNOW_BALL, BLOCK_COST)) {
                    ...
                    } else event.getPlayer().getInventory().addItem(new Item(Material.SNOW_BLOCK).toItemStack());
                }
                event.getPlayer().updateInventory();
            }
            ...
            if (event.getItem().getType() == Material.DIAMOND_SPADE && event.getPlayer().getInventory().contains(Material.SNOW_BLOCK, MAN_COST)) {
                ...
                if (DataPlayer.getUser(event.getPlayer()).remove(Material.SNOW_BLOCK, MAN_COST)) {
